...
私も昔はそうでした。
修正や障害対応の調査のため、コードを1行1行、場合によっては1文字ずつ読んでいましたが、なかなか終わりません。それに読む場所が収集つかなくなり、「結局、何を調べていたんだっけ？」と迷い、調査が終わらず、徒労感に苛まれるが多々ありました。
...
=== 完璧主義が邪魔をする
コードを読む時、@<strong>{全てを完全に理解しようとする必要はありません。}コードの多くが定型の記述であり、8割程度の理解で、該当のコードが何をしているか、理解することが可能だからです。そのため、コードを完璧に理解する必要はありません。
...
そのような変数名・関数名・クラス名を見ると、変数名・関数名・クラス名を見るだけで、@<strong>{「何をしようとしているのか」}を語っていることが多いです。読みやすい名前だと、コードを読み進めるだけで、処理の一連の流れである「ストーリー」が頭に入ってきます。
 ...
 * 繰り返し現れる定型的なコードパターン（for...range や if err != nil など）は、見つけた瞬間に @<strong>{「意味のチャンク」}として脳にインプットしておくと、素早く読むことが可能です。
...
== ツールがもたらす超速読の世界
コードはIDEやAIを使うと、より速読のスピードを高めることができます。
そのテクニックを以下に記載します。

=== IDEは「読むための相棒」
 * @<strong>{「Go to Definition」}と@<strong>{「Find Usages」}で、コードを瞬時にナビゲートする
 * @<strong>{「Call Hierarchy」}で関数の呼び出しツリーを視覚化する
 * デバッガーを使って、動的にコードの流れを追跡する

=== AIアシスタントを使いこなす
 * 複雑なロジックや正規表現を、AIに要約させたり、解読させる
...
本章で学んだことを日々の開発に活かし、@<strong>{「コードを読むのが楽しい」}と感じられるようになれば幸いです。
